...
long EditDistance_NW_Iter(char* A, size_t lengthA, char* B, size_t lengthB) {
   ...
   long *memoM = malloc((M+1) * sizeof(long));
   ...
   for (int j = N; j>= 0; j--) {
      ...
      if (j != N) {
         saver = (isBase(Y[j]) ? INSERTION_COST : 0)  + saver;
      }
      for (int i = M-1; i>= 0; i --) {
         ...
         if (j != N) {
            ...
            if (!isBase(X[i])) {
               ...
               min = ( isUnknownBase(X[i]) ?  SUBSTITUTION_UNKNOWN_COST : ( isSameBase(X[i], Y[j]) ? 0 : SUBSTITUTION_COST ) ) + memoM[i+1]; 
               { long cas2 = INSERTION_COST + memoM[i] ;      
               if (cas2 < min) min = cas2 ;
               }
               { long cas3 = INSERTION_COST + saver ;      
               if (cas3 < min) min = cas3 ;
               }
            }
            memoM[i+1] = saver;
            ...
            memoM[i] = (isBase(X[i]) ? INSERTION_COST : 0) + memoM[i+1];
         }
...
long EditDistance_NW_Iter2(char* A, size_t lengthA, char* B, size_t lengthB) {
   ...
   long *memoM = malloc((M+1) * sizeof(long));
   long *memoN = malloc((N+1) * sizeof(long));
   ...
   for (int j = N-1; j>=0; j --) {
      memoN[j] = (isBase(Y[j]) ? INSERTION_COST : 0)  + memoN[j+1];
   }
...
void compute(int J, int I, int M, int N, int Km, int Kn, long *memoM, long *memoN) {


   int Inew = (I==M)?-1:0 + I;
   int Jnew = (J==N)?-1:0 + J;

   for (int j = Jnew; j>max(J-Km, -1); j--) {

      for (int i = Inew; i>max(I-Kn, -1); i--) {
...
long EditDistance_NW_Aware(char* A, size_t lengthA, char* B, size_t lengthB) {
   ...
   // gotta change : J from N to 0 and skip the N (same for I) to step every ...

   for (int J = N; J >= 0; J -= Kn) {


      for (int I = M; I >= 0; I -= Km) {
